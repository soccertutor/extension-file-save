...
void fs_requestSavePath(const char* name, const char* mime, value onSelect, value onCancel) {
...
void fs_saveFile(const char* src, const char* name, const char* mime, bool as_copy, value callback) {
	filesave_saveFile(src, name, mime, as_copy, callback);
}
DEFINE_PRIME5v(fs_saveFile);

extern "C" void extension_file_save_main() {
	val_int(0);
}
DEFINE_ENTRY_POINT(extension_file_save_main);

extern "C" int extension_file_save_register_prims() {
	return 0;
}
